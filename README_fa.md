...
- **بک‌اند**: Rust، با استفاده از [`portable-pty`](https://docs.rs/portable-pty) که فایل واقعیِ `aether` (نسخه‌ی 1.4.0) را داخل یک شبه‌ترمینالِ واقعی اجرا می‌کند. تنظیمات انتخابی شما — پروتکل، حالت اسکن، نسخه‌ی IP، ترنسپورتِ MASQUE (HTTP/3 یا HTTP/2)، پروفایل مبهم‌سازی و اتصال سریع — از همان اول به‌صورت فلگ‌های خط فرمان و متغیر محیطی به Aether داده می‌شود، پس معمولاً هیچ سؤال تعاملی‌ای ظاهر نمی‌شود؛ با این حال یک ریسه در پس‌زمینه خروجی را می‌پاید و اگر سؤالی هم پیش بیاید جوابش را می‌دهد — و در همان حال هر خط را زنده به پنل لاگِ برنامه می‌فرستد.
